#!/usr/bin/env python3
"""
Canvas MCP HTTP Server - SECURE VERSION
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('/var/log/canvas-mcp-security.log'),
        logging.StreamHandler()
    ]
)
security_logger = logging.getLogger('canvas-mcp-security')
...
class MultiTenantCanvasHTTPServer:
    ...
    def __init__(self):
        ...
        self.max_requests_per_minute = 60  # Rate limiting per session
        self.max_requests_per_ip_per_minute = 100  # IP-based rate limiting
    ...
    def check_ip_rate_limit(self, client_ip: str) -> bool:
        ...
        if self.ip_request_counts[client_ip]['count'] >= self.max_requests_per_ip_per_minute:
            security_logger.warning(f"IP rate limit exceeded for {client_ip}")
            return False
    ...
    def check_rate_limit(self, session_id: str) -> bool:
        ...
        if self.request_counts[session_id]['count'] >= self.max_requests_per_minute:
            security_logger.warning(f"Session rate limit exceeded for {session_id[:8]}...")
            return False
...
def main():
    """Main entry point."""
    print("🚀 Starting Canvas MCP HTTP Server (SECURE VERSION)...", file=sys.stderr)
    ...
    print(f"🔍 Security logging enabled: /var/log/canvas-mcp-security.log", file=sys.stderr)
