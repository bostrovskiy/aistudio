#!/usr/bin/env python3
"""
Canvas MCP HTTP Server - PRODUCTION VERSION
HTTP/HTTPS endpoint for MCP server with enhanced security
...
- Cryptographically secure sessions: Enhanced session security
"""
...
def setup_logging():
    """Setup security logging with proper permissions."""
    log_file = os.getenv("SECURITY_LOG_FILE", "/var/log/canvas-mcp-security.log")
    ...
    if not os.path.exists(log_dir):
        try:
            ...
            log_file = "canvas-mcp-security.log"
    ...
    return logging.getLogger('canvas-mcp-security')

class MultiTenantCanvasHTTPServer:
    """Multi-tenant Canvas MCP Server with HTTP/HTTPS endpoint and enhanced security."""
    ...
    def __init__(self):
        ...
        self.max_requests_per_ip_per_minute = int(os.getenv("MAX_REQUESTS_PER_IP_PER_MINUTE", "100"))
        self.request_counts: Dict[str, Dict[str, Any]] = {}  # Track request rates
        self.ip_request_counts: Dict[str, Dict[str, Any]] = {}  # Track IP request rates
        
        # Security logging
        self.security_logger = setup_logging()
    ...
    def check_ip_rate_limit(self, client_ip: str) -> bool:
        ...
        if client_ip not in self.ip_request_counts:
            self.ip_request_counts[client_ip] = {'count': 0, 'window_start': now}
        
        # Reset counter if window has passed
        if now - self.ip_request_counts[client_ip]['window_start'] > timedelta(minutes=1):
            self.ip_request_counts[client_ip] = {'count': 0, 'window_start': now}
        
        # Check if limit exceeded
        if self.ip_request_counts[client_ip]['count'] >= self.max_requests_per_ip_per_minute:
            self.security_logger.warning(f"IP rate limit exceeded for {client_ip}")
            return False
        
        # Increment counter
        self.ip_request_counts[client_ip]['count'] += 1
        return True
    ...
    def check_rate_limit(self, session_id: str) -> bool:
        ...
        if self.request_counts[session_id]['count'] >= self.max_requests_per_minute:
            self.security_logger.warning(f"Session rate limit exceeded for {session_id[:8]}...")
            return False
    ...
    def log_security_event(self, event_type: str, details: Dict[str, Any], client_ip: str):
        """Log security events for audit trail."""
        self.security_logger.info(f"SECURITY_EVENT: {event_type} from {client_ip} - {details}")
    
    async def authenticate_user(self, api_token: str, api_url: str, client_ip: str) -> Optional[str]:
        """
        ...
        try:
            ...
            if not self.validate_input(api_token, "api_token"):
                self.log_security_event("INVALID_TOKEN_FORMAT", {"api_token_length": len(api_token)}, client_ip)
                return None
            
            if not self.validate_input(api_url, "api_url"):
                self.log_security_event("INVALID_URL_FORMAT", {"api_url": api_url}, client_ip)
                return None
            ...
            async with httpx.AsyncClient() as client:
                ...
                if len(existing_sessions) >= self.max_sessions_per_user:
                    ...
                    self.log_security_event("SESSION_LIMIT_EXCEEDED", {"user_id": user_data.get("id")}, client_ip)
                ...
                self.log_security_event("AUTHENTICATION_SUCCESS", {
                    "user_id": user_data.get("id"),
    ...
    def get_user_session(self, session_id: str) -> Optional[Dict[str, Any]]:
        ...
        if datetime.now() - session["last_activity"] > self.session_timeout:
            ...
            self.log_security_event("SESSION_EXPIRED", {"session_id": session_id[:8] + "..."}, session.get("client_ip", "unknown"))
            return None
    ...
    async def make_canvas_request(self, session_id: str, method: str, endpoint: str, **kwargs) -> Any:
        ...
        try:
            ...
            if not endpoint.startswith('/') or '..' in endpoint:
                self.log_security_event("PATH_TRAVERSAL_ATTEMPT", {"endpoint": endpoint}, session.get("client_ip", "unknown"))
                return {"error": "Invalid endpoint"}
            
            # Limit request size
            if kwargs.get("data") and len(str(kwargs["data"])) > 10000:
                self.log_security_event("LARGE_REQUEST_ATTEMPT", {"size": len(str(kwargs["data"]))}, session.get("client_ip", "unknown"))
    ...
    def setup_routes(self):
        """Setup FastAPI routes with enhanced security."""
        ...
        async def security_middleware(request: Request, call_next):
            ...
            if not self.check_ip_rate_limit(client_ip):
                self.log_security_event("IP_RATE_LIMIT_EXCEEDED", {"client_ip": client_ip}, client_ip)
                return JSONResponse(
                    status_code=429,
            ...
            # Log request
            self.log_security_event("REQUEST_RECEIVED", {
        ...
        async def authenticate(request: AuthenticateRequest, http_request: Request):
            """Authenticate with Canvas credentials."""
            client_ip = self.get_client_ip(http_request)
            session_id = await self.authenticate_user(request.api_token, request.api_url, client_ip)
            if session_id:
                session = self.user_sessions[session_id]
                return {
                    "success": True,
        ...
        async def get_profile(request: SessionRequest, http_request: Request):
        ...
        async def list_courses(request: ListRequest, http_request: Request):
        ...
        async def get_course_details(course_id: str, request: SessionRequest, http_request: Request):
        ...
        async def list_assignments(request: CourseRequest, http_request: Request):
        ...
        async def get_assignment_details(assignment_id: str, request: AssignmentRequest, http_request: Request):
        ...
        async def list_discussions(request: DiscussionListRequest, http_request: Request):
        ...
        async def get_discussion_details(discussion_id: str, request: DiscussionRequest, http_request: Request):
        ...
        async def list_announcements(request: CourseRequest, http_request: Request):
        ...
        @self.app.get("/grades")
        async def get_grades(request: CourseRequest, http_request: Request):
        ...
        async def list_calendar_events(request: CalendarRequest, http_request: Request):
        ...
        @self.app.get("/search")
        async def search_courses(request: SearchRequest, http_request: Request):
        ...
        async def get_session_info(request: SessionRequest, http_request: Request):
        ...
        async def logout(request: SessionRequest, http_request: Request):
            ...
            if request.session_id in self.user_sessions:
                ...
                self.log_security_event("LOGOUT", {"user_name": user_name}, client_ip)
...
def main():
    """Main entry point."""
    print("🚀 Starting Canvas MCP HTTP Server (PRODUCTION VERSION)...", file=sys.stderr)
    ...
    print("🛡️ Enhanced security features: Rate limiting, input validation, data anonymization, security logging", file=sys.stderr)
    ...
    print(f"🔍 Security logging enabled: {os.getenv('SECURITY_LOG_FILE', '/var/log/canvas-mcp-security.log')}", file=sys.stderr)
