#!/usr/bin/env python3
"""
Canvas MCP HTTPS Secure Server with Local Credential Storage
This server provides MCP protocol over HTTPS with secure local credential storage
"""
...
class CanvasMCPHTTPSSecureStoredServer:
    """Secure HTTPS MCP server with local credential storage."""
    ...
    def store_credentials(self, api_token: str, api_url: str, institution_name: str = ""):
        ...
        try:
            ...
            # Store in keyring
            keyring.set_password("canvas-mcp", key, encoded_creds)
    ...
    def load_credentials(self):
        ...
        try:
            ...
            # We need to know the API URL to get the right key
            # For now, we'll prompt if not found
    ...
    def load_credentials_for_url(self, api_url: str):
        ...
        try:
            key = self.get_credential_key(api_url)
            encoded_creds = keyring.get_password("canvas-mcp", key)
    ...
    def handle_mcp_request(self, request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if method == "initialize":
            ...
            if tool_name == "authenticate_canvas":
                ...
                try:
                    key = self.get_credential_key(api_url)
                    keyring.delete_password("canvas-mcp", key)
                    return {
                        "jsonrpc": "2.0",
                        "id": request.get("id"),
                        "result": {
...
def main():
    ...
    server = CanvasMCPHTTPSSecureStoredServer(base_url=args.base_url)
    server.run()
